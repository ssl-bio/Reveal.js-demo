...
function getColor(){
    clrMain=getComputedStyle(document.body).getPropertyValue('--r-main-color');
    clrBg=getComputedStyle(document.body).getPropertyValue('--r-background-color');
    clrLink=getComputedStyle(document.body).getPropertyValue('--r-link-color');
    

    if (clrMain.startsWith('#')){
	clrMain=clrNorm(clrMain);
    } else {
	clrMain=rgba2hexa(clrMain);
	clrMain=clrNorm(clrMain);
    }
    ...
    let bgMsg=clrBg+"bf"; // bf = 75% opacity
    let bgColorA=clrLink+"26"; // 26 = 15% opacity
    let bgColorB=clrMain+"40"; // 40 = 25% opacity
...
function ilegend(){
	...
	if (isec.style.backgroundColor === 'transparent') {
	    // Set the background color
	    isec.style.backgroundColor = bgMsg;
	    idiv.classList.toggle('out');
	} else {
	    // Remove the background color
	    isec.style.backgroundColor = 'transparent';
	    idiv.classList.toggle('out');
	}
	updateBGcol();
    }
